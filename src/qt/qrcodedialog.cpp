...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("FID")
    updateDisplayUnit();
}
...
{
    QString ret = QString("FidChain:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FID unit input in lnReqAmount, we generate the URI with FID as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::FID, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
