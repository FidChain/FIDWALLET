...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("FidChain"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::FID, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert FidChain:// to FidChain:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("FidChain://"))
    {
        uri.replace(0, 12, "FidChain:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FidChain.lnk";
}
...
{
    return GetAutostartDir() / "FidChain.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FidChain") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  FidChain [" + tr("command-line options") + "]                     " + "\n";
